...
import ch.xxx.manager.prod.usecase.service.ProdAppInfoService;
...
@ComponentScan(basePackages = "ch.xxx.manager",excludeFilters = @Filter(type = FilterType.ASSIGNABLE_TYPE, classes = ProdAppInfoService.class))
public class PortfolioControllerTest extends BaseControllerTest {
	...
	public void findPortfolioByIdNotFound() throws Exception {
		...
		this.mockMvc.perform(get("/rest/portfolio/id/{portfolioId}",1L).header(JwtUtils.AUTHORIZATION, String.format("Bearer %s", myToken)).servletPath("/rest/portfolio"))
