...
export class NewPortfolioComponent {
  ...
  constructor(public dialogRef: MatDialogRef<OverviewComponent>,
		...
		this.portfolioForm = this.fb.group({
			...
			[FormFields.CreatedAt]: [DateTime.fromISO(this.data.portfolio.createdAt), Validators.required]
		}, {
  ...
  onAddClick(): void {
	...
	const portfolio: Portfolio = {id: null,createdAt: new Date(createdAt.toMillis()).toISOString(),
		...
		symbols: [], portfolioElements: [], userId: (this.tokenService.userId as number), year1: null, year10: null, year2: null, year5: null }; 		
  ...
  validate(formGroup: FormGroup) {
	if (formGroup.get(FormFields.PortfolioName).touched) {
		...
		if(myValue && myValue.trim().length > 4) {
			...
			formGroup.get(FormFields.PortfolioName).setErrors({ MatchPassword: true });
